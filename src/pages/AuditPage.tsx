...
export function AuditPage() {
  return <div className="page-stack"><header className="page-heading"><div><div className="eyebrow">EVIDENCE, NOT GUESSWORK</div><h1>资料审计</h1><p>结论来自 2026-07-28 在 Chrome 中实际查看的夸克目录；“目录存在”、“实际可播”与“内容完整”是三个不同证据层级。</p></div><a className="button button--secondary" href={quarkRootUrl} target="_blank" rel="noopener noreferrer"><ExternalLink />打开私有夸克</a></header>
    ...
    <section className="panel audit-table"><div className="section-heading"><div><span>AUDIT DETAILS</span><h2>逐项覆盖结论</h2></div><ShieldCheck /></div><div className="audit-rows">{RESOURCE_AUDIT.items.map((item) => { const meta = STATUS[item.status]; const Icon = meta.icon; return <article key={item.id} data-testid={`audit-${item.id}`} data-audit-status={item.status} className={`audit-row audit-row--${item.status}`}><Icon /><div><h3>{item.label}</h3><p>{item.detail}</p></div><b>{meta.label}</b></article>; })}</div><p className="audit-basis">审计依据：{RESOURCE_AUDIT.auditBasis}</p></section>
