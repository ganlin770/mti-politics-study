...
function QuestionCard({ question }: { question: SelfTestQuestion }) {
  ...
  return <article className={`question-card${attempt ? (attempt.correct ? ' is-correct' : ' is-wrong') : ''}`}><div className="question-top"><span>{question.type === 'single' ? '单选' : '多选'}</span><em>{question.sourceLabel}</em>{attempt ? attempt.correct ? <CheckCircle2 /> : <CircleAlert /> : null}</div><h3>{question.stem}</h3><div className="option-list">{question.options.map((option) => { const chosen = selected.includes(option.id); const answer = attempt && question.correctOptionIds.includes(option.id); return <button key={option.id} type="button" aria-pressed={chosen} disabled={Boolean(attempt)} className={`quiz-option${chosen ? ' is-selected' : ''}${answer ? ' is-answer' : ''}`} onClick={() => toggle(option.id)}><b>{option.id}</b><span>{option.text}</span>{answer ? <Check /> : chosen && attempt && !attempt.correct ? <X /> : null}</button>; })}</div>{attempt ? <div className="explanation"><b>{attempt.correct ? '回答正确' : '需要订正'}</b><p>{question.explanation}</p></div> : <button className="button button--primary answer-button" type="button" disabled={!selected.length} onClick={() => answerQuestion(question, selected)}>提交答案</button>}</article>;
}
