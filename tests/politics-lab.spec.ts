...
async function openPrimarySection(page: Page, projectName: string, name: '练题' | '审计') {
  if (projectName.startsWith('mobile')) {
    ...
    if (name === '练题') {
      await mobileNav.getByRole('link', { name: '练题' }).click();
      return;
    }

    await mobileNav.getByRole('button', { name: '更多' }).click();
    await expect(page.getByRole('dialog', { name: '更多' })).toBeVisible();
    await page.getByRole('dialog', { name: '更多' }).getByRole('link', { name: '资料审计' }).click();
    return;
  }
...
test('renders the study-first home screen at desktop and mobile sizes', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('heading', { name: '今天只做三件事' })).toBeVisible();
  await expect(page.getByTestId('today-chain')).toContainText('马原 · 导论');
  await expect(page.getByTestId('today-chain')).toContainText('导论：单选1—7、多选1—2');
...
test('navigates, answers an original question, and exposes the audited resource gaps', async ({ page }, testInfo) => {
  ...
  await expect(page.getByText('原创定位自测（非肖1000、非历年真题）').first()).toBeVisible();
  ...
  await expect(page.locator('.audit-row--partial').filter({ hasText: '肖1000逐题视频讲解' })).toBeVisible();
  await expect(page.locator('.audit-row--missing')).toHaveCount(3);
  await expect(page.locator('vite-error-overlay')).toHaveCount(0);
...
test('keeps a later active lesson aligned with its external practice log', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.practice-log-panel')).toContainText('哲学及其基本问题 · 肖1000账本');
